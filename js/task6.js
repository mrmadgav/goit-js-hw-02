...
do {
  //начало
  input = prompt("Введите число"); // промт
  inputValid = parseInt(input); // делаем числом
  if (Number.isNaN(inputValid) === false) {
    // если число
    numbers.push(input); // пушим в массив
    console.log(numbers);
  } else if (input !== null) {
    // если Cancel
    alert(message);
...
} while (input !== null); // пока вводятся числа
for (let i = 0; i < numbers.length; i++) {
  // счетчик
  // debugger;
  total += parseInt(numbers[i]); // складываем массив
}
console.log(total); // выводим сумму
